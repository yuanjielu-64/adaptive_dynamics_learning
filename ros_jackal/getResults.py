...
def calculate_environment_statistics(combined_df, processed_files, use_existing_nav_metric=False):
    ...
    for world_id, group in combined_df.groupby('world_id'):
        ...
        if total_episodes > 10:  # Only apply trimming if we have enough data
            ...
            trimmed_times = sorted_times.iloc[5:-5]  # Remove first 5 and last 5
            avg_time = trimmed_times.mean()
        else:
            avg_time = group['Time'].mean()  # Use all data if insufficient samples
...
def analyze_status_distribution(sorted_results, top_n_per_env=20):
    ...
    all_avg_times = [r['avg_success_time'] for r in sorted_results]
...
if __name__ == "__main__":
    ...
    # CONFIGURATION
    # ============================================================================
    baseline_dir = "buffer/model"    # Directory containing baseline CSV files
